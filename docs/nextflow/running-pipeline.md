...
### Step 1 — Make sure validation completed successfully

Before running the pipeline, ensure that the validation step has completed successfully. The `data/valid/` folder should contain:

- `data/valid/validated_params.json` — the parameter file consumed by Nextflow
- `data/valid/run_YYYYMMDD_HHMMSS/` — a timestamped directory with the validated input files

If these are missing, run validation first (see the [Validation Overview](../validation/OVERVIEW.md) for instructions and details).
