...
The `-params-file data/valid/validated_params.json` flag loads the parameters generated by the validation step. It overrides the defaults in `nextflow.config` and automatically sets which workflows to run (`run_syri`, `run_illumina`, `run_nanopore`, `run_pacbio`) and the validated file paths — so no manual parameter flags are needed for file inputs.
