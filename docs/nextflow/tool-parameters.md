...
### mosdepth (flank depth calculation)

**Module:** `modules/mapping.nf`

```bash
mosdepth --threads ${task.cpus} --no-per-base --by <regions.bed> <prefix> <bam>
```

| Parameter | Value | mosdepth Default | Description |
|-----------|-------|------------------|-------------|
| `--threads` | `${task.cpus}` | 0 | Number of BAM decompression threads. Set dynamically from the Nextflow task allocation. |
| `--no-per-base` | *(flag)* | off | Skip per-base depth output. Only region-level summary is needed for flank coverage, so per-base output is disabled to save disk and time. |
| `--by` | `<regions.bed>` | *(none)* | BED file defining the regions to compute depth over. The pipeline supplies 100 bp upstream and 100 bp downstream flanks around each SV event, generated by `build_sv_flank_bed`. |

**Rationale:** mosdepth is used to add local read-depth context around each structural variant call from short-read and long-read pipelines. The `--no-per-base` flag is set because only the mean depth per flank region is needed (not nucleotide-resolution depth), which significantly reduces output size and I/O. The `--by` flag restricts computation to the pre-built flank BED regions rather than computing genome-wide depth.

**Output columns added to SV TSVs:**
...
| mosdepth | Short-read, Long-read | `mapping.nf` | `--no-per-base`, `--by <flanks.bed>` |
