...
### `mod_contig_[0..4]/*.delta`
Raw alignment difference files between reference and each modified contig (generated by `nucmer`/MUMmer).

### `mod_contig_[0..4]/*.filtered.coords`
Filtered alignment coordinates showing high-confidence matches and structural differences for each contig.

### `mod_contig_[0..4]/*_filtered.delta`
Cleaned and filtered delta files used for downstream structural comparison.
...
### `assembly/assembly_concat.vcf`
Concatenated VCF across all contigs, produced by `bcftools concat`.
