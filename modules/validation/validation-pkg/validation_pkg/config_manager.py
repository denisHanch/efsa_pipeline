...
class ReadConfig(BaseValidatorConfig):
    ...
    def __post_init__(self):
        if self.ngs_type is not None and self.ngs_type not in ["illumina", "ont", "pacbio"]:
...
class ConfigManager:
    ...
    def _parse_read_config(value: Any, field_name: str, config_dir: Path, output_dir: Path, global_options: Dict[str, Any] = None) -> ReadConfig:
        ...
        ngs_type = value.get('ngs_type') if isinstance(value, dict) else None

        if not ngs_type:
            # Determine the effective validation level for this file
            file_level = value.get('validation_level') if isinstance(value, dict) else None
            effective_level = file_level or (global_options or {}).get('validation_level', 'strict')

            if effective_level != 'strict':
                raise ConfigurationError(
                    f"{field_name}: 'ngs_type' is required when validation_level is "
                    f"'{effective_level}'. Must be one of: illumina, ont, pacbio. "
                    f"Omit 'ngs_type' only with validation_level='strict' for autodetection."
                )
            # strict mode: leave ngs_type=None — the ReadValidator will detect from content
            ngs_type = None
