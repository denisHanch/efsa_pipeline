...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --threads <int>              Number of threads (overrides config.json when set)"
            echo "  --validation-level <level>   'strict', 'trust', or 'minimal' (overrides config.json)"
            echo "  --logging-level <level>      'DEBUG', 'INFO', 'WARNING', or 'ERROR' (overrides config.json)"
            echo "  --type <type>                'prokaryote' or 'eukaryote' (overrides config.json)"
