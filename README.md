...
3. **Use the GitHub token provided in the Slack channel**, then start the pipeline:
...
* `unmapped/` — Reads not mapped to the plasmid and not mapped to the reference genome
...
* `unmapped/` — Fastq file containing reads that failed to align to the modified genome
...
* `unmapped/` — Fastq file containing reads that did not map to the plasmid
