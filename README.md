...
At the first step two processes vcf_to_table_long and vcf_to_table are used to extract all important columns from vcf to a tsv table.

- vcf_to_table uses if else to run on assembly pipeline and short-read pipeline
- vcf_to_table_long focus es on columns generated by survivor (debbreak, sniffles, and cute_sv combined variants)
- both processes use bcftools query to extract the values of the interest

At the second step python script is used to parse the output of 3 tables into one comprehensive xlsx table.
